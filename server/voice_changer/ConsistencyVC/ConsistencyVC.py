...
from voice_changer.ConsistencyVC.ConsistencyVCSettings import ConsistencyVCSettings
...
from voice_changer.ConsistencyVC.pipeline.PipelineGenerator import createPipeline
...
class ConsistencyVC(VoiceChangerModel):
    def __init__(self, params: VoiceChangerParams, slotInfo: ConsistencyVCModelSlot):
        logger.info("[Voice Changer] [ConsistencyVC] Creating instance ")
        self.voiceChangerType = "ConsistencyVC"
        ...
        self.settings = ConsistencyVCSettings()
        self.params = params
    ...
    def initialize(self):
        logger.info("[Voice Changer] [ConsistencyVC] Initializing... ")
        ...
        logger.info("[Voice Changer] [ConsistencyVC] Initializing... done")
    ...
    def update_settings(self, key: str, val: int | float | str):
        logger.info(f"[Voice Changer][ConsistencyVC]: update_settings {key}:{val}")
    ...
    def inference(
        ...
        audio = np.frombuffer(receivedData, np.int16).flatten().astype(np.float32) / 32768.0
        #write(os.path.join("tmp_dir", "{}.wav".format("input")), 48000, test)

        audio16k = resampy.resample(audio, self.inputSampleRate, self.processingSampleRate)
        #write(os.path.join("tmp_dir", "{}.wav".format("audio16k")), 16000, audio16k)
        ...
        print("audio2",audio)
        # audio = torchaudio.functional.resample(receivedData, self.slotInfo.samplingRate, 16000, rolloff=0.99)
        ...
        try:
            ...
            print("audio_out",audio_out[0][0].data.cpu().float().numpy())
            audio_out = audio_out[0][0].data.cpu().float().numpy()
            #audio_out.detach().cpu().numpy()

            audio_out48k = resampy.resample(audio_out, self.processingSampleRate, self.inputSampleRate)
            audio_out48k = cast(AudioInOutFloat, audio_out48k)
            result = (audio_out48k * 32767.5).astype(np.int16)
